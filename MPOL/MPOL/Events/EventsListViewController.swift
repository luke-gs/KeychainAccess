...
open class EventsListViewController: FormBuilderViewController {
    ...
    required public init(viewModel: EventListViewModelType, incidentsManager: IncidentsManager) {
    ...
    open override func construct(builder: FormBuilder) {
        ...
        builder += eventsList.map { displayable in
            ...
            return SubtitleFormItem(title: title, subtitle: subtitle, image: image)
                ...
                .onSelection( { cell in
                    guard let event = self.viewModel.event(for: displayable) else {
                        return
                    }
                    // produce a new EventSplitViewController by providing a detailsViewModel generated by this view controller's view model, taking the event for this displayable
                    let viewController = EventSplitViewController(viewModel: self.viewModel.detailsViewModel(for: event))
    ...
    @objc private func createNewEvent() {
        ...
        viewController.didSelectIncident = { incident in
            guard let event = self.viewModel.eventsManager.create(eventType: .blank) else { return }

            let viewController = EventSplitViewController(viewModel: self.viewModel.detailsViewModel(for: event))

            if let incidentType = incident {
                let incident = self.incidentsManager.create(incidentType: incidentType, in: event)
            } else {
