...
class TestPersonStrategy: EntityRetrieveStrategy {
    func retrieveUsingReferenceEntity(_ entity: MPOLKitEntity) -> Promise<[EntityState]>? {
        ...
        if entity.source == .pscore {
        ...
        } else if let externalId = entity.externalIdentifiers?[.rda] {
            ...
            return APIManager.shared.fetchEntityDetails(in: MPOLSource.rda, with: request)
                .then { person -> Promise<[EntityState]> in
                    return Promise.value([EntityState.detail(person)])
            }
        } else {
            ...
            return APIManager.shared.searchEntity(in: MPOLSource.rda, with: request)
